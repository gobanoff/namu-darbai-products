  ...
  products.map((product) => {
    ...
    <td>
     <button onclick="deleteProduct(event, ${product.id});"
      class="btn btn-danger">Ištrinti</button>    
     <button onclick="editProduct(event, ${product.id});"class="btn btn-primary">Edit</button>
     <button onclick="saveEditedProduct(event, ${product.id});"type="button" class="btn btn-info">Save</button>
     </td>
    </tr>`;
  });
...
function saveEditedProduct(e, id) {
  const index = products.findIndex((val) => val.id === id);
  let product = products[index];
  document.querySelectorAll("form .form-control").forEach((el) => {
    product[el.getAttribute("name")] = el.value;
